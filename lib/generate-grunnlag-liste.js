...
module.exports = function generateGrunnlagListe (data) {
  ...
  if (data.bosted.bosted == 'folkeregistrert' && validPostnummerFerry.indexOf(data.dsfData.POSTS.toString()) > -1) {
    liste.push(grunnlagBatFerge)
  }

  if (data.bosted.bosted == 'delt') {
    if (validPostnummerFerry.indexOf(data.dsfData.POSTS.toString()) > -1 || validPostnummerFerry.indexOf(data.bosteddelt.POSTS.toString()) > -1) {
      liste.push(grunnlagBatFerge)
    }
  }

  if (data.bosted.bosted == 'hybel' && validPostnummerFerry.indexOf(data.bostedhybel.POSTS.toString()) > -1) {
    liste.push(grunnlagBatFerge)
  }
